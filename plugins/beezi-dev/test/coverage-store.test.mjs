...
test('coverage — a 2xx in the documented shape becomes a Map of stored prefixes', async () => {
  ...
  const map = await fetchCoverage(['s1', 's2'], 'tok', { postJsonImpl: post.impl });
...
test('coverage — an id absent from a valid 2xx body is a CONFIRMED ZERO, not unavailable', async () => {
  ...
  const map = await fetchCoverage(['s1'], 'tok', { postJsonImpl: post.impl });
...
test('coverage — a transport throw returns null, never an empty Map', async () => {
  const map = await fetchCoverage(['s1'], 'tok', {
...
test('coverage — a non-2xx returns null', async () => {
  const map = await fetchCoverage(['s1'], 'tok', { postJsonImpl: async () => ({ status: 500, json: async () => ({}) }) });
  assert.equal(map, null);
});
...
test('coverage — a 2xx without the documented coverage key returns null (older server)', async () => {
  for (const body of [{}, { ok: true }, { coverage: null }, { coverage: [] }, { coverage: 'nope' }]) {
    const map = await fetchCoverage(['s1'], 'tok', { postJsonImpl: async () => ok(body) });
...
test('coverage — a line count we do not understand makes the run unavailable, not zero', async () => {
  for (const value of ['12', -1, 1.5, {}, true]) {
    const map = await fetchCoverage(['s1'], 'tok', { postJsonImpl: async () => ok({ coverage: { s1: value } }) });
...
test('coverage — 201 ids are asked in two batches', async () => {
  ...
  const post = recordingPost(() => ok({ coverage: {} }));
  const map = await fetchCoverage(ids, 'tok', { postJsonImpl: post.impl });
...
test('coverage — no ids asks nothing and is trivially available', async () => {
  const post = recordingPost(() => ok({ coverage: {} }));
  const map = await fetchCoverage([], 'tok', { postJsonImpl: post.impl });
...
test('checkpoints — survive the prune that sweeps state/ and queue/', (t) => {
  ...
  const record = loadCoverageCheckpoints(binding);
  recordCoverageCheckpoint(record, 's1', 900);
  assert.equal(saveCoverageCheckpoints(record), true);
  ...
  fs.writeFileSync(path.join(queueDir(), 's1_1-9.json'), '{}');
  const old = new Date(Date.now() - 400 * 24 * 60 * 60 * 1000);
  fs.utimesSync(path.join(stateDir(), 's1.json'), old, old);
  fs.utimesSync(path.join(queueDir(), 's1_1-9.json'), old, old);
  pruneStale();

  assert.equal(fs.existsSync(path.join(stateDir(), 's1.json')), false, 'the cursor is gone');
  assert.equal(fs.existsSync(coverageCheckpointFile()), true, 'the coverage checkpoint is not');
  assert.equal(checkpointLineFor(loadCoverageCheckpoints(binding), 's1'), 900);
  // And the file lives at the data-root level, not inside either swept directory.
  assert.equal(path.dirname(coverageCheckpointFile()), home);
});

test('checkpoints — a different machine identity discards the record rather than trusting it', (t) => {
  tmpHome(t);
  const record = loadCoverageCheckpoints(currentBinding('machine-a'));
  recordCoverageCheckpoint(record, 's1', 900);
  saveCoverageCheckpoints(record);
  const other = loadCoverageCheckpoints(currentBinding('machine-b'));
...
test('checkpoints — a different environment or API base discards the record', (t) => {
  ...
  recordCoverageCheckpoint(record, 's1', 900);
  saveCoverageCheckpoints(record);
  assert.equal(checkpointLineFor(loadCoverageCheckpoints({ ...binding, environment: 'staging' }), 's1'), null);
  assert.equal(checkpointLineFor(loadCoverageCheckpoints({ ...binding, apiBase: 'https://elsewhere/api' }), 's1'), null);
  assert.equal(checkpointLineFor(loadCoverageCheckpoints(binding), 's1'), 900, 'the matching binding still reads');
});

test('checkpoints — never walk backwards', (t) => {
  tmpHome(t);
  const record = loadCoverageCheckpoints(currentBinding('m'));
  recordCoverageCheckpoint(record, 's1', 900);
...
test('checkpoints — a corrupt file reads as empty instead of throwing', (t) => {
  ...
  fs.writeFileSync(coverageCheckpointFile(), '{ not json');
  const record = loadCoverageCheckpoints(currentBinding('m'));
