...
//
// Exported for lib/diagnostics.mjs, which records `errorCode` as a dimension: `fetch failed` is the
// single most common shape a hook crashes with, and a bare `error.code` read misses every one of
// them because the errno lives on the cause. The diagnostics record shapes whatever comes back
// through its own identifier allowlist, so nothing here needs to know about redaction.
export function codeOf(error) {
  const direct = (error || {}).code;
  if (direct !== undefined && direct !== null) return direct;
  return orDefault(((error || {}).cause || {}).code, null);
}
...
function isTimeout(error) {
  const name = orDefault((error || {}).name, ((error || {}).cause || {}).name);
  return name === 'AbortError' || name === 'TimeoutError';
}

function isBadJson(error) {
  return error instanceof SyntaxError || /\bJSON\b/i.test(String(orDefault((error || {}).message, '')));
}
...
export function friendlyMessage(error, { env = process.env } = {}) {
  ...
  const raw = String(orDefault(orDefault((error || {}).message, error), '')).trim();
  if (env && env.BEEZI_DEBUG && raw) return `Something went wrong: ${raw}`;
